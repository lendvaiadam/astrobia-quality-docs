...
export class AudioManager {
    ...
    startMusic() {
        ...
        // Debug: console.log('[Audio] spaceSound:', !!this.spaceSound, 'buffer:', !!this.spaceSound?.buffer, 'ready:', this.spaceSoundReady);
        ...
        if (this.spaceSound && this.spaceSoundReady && !this.spaceSound.isPlaying) {
            try {
                ...
                console.log("[Audio] Space music started");
            } catch (e) {
                console.warn("[Audio] play() threw error:", e);
            }
        }
        // Only warn once, not every frame
        // else { console.warn('[Audio] Cannot play space music'); }

        if (this.atmoSound && this.atmoSoundReady && !this.atmoSound.isPlaying) {
            this.atmoSound.play();
            console.log("[Audio] Atmosphere sound started");
        }
        ...
        this.pendingUnitSounds = [];
        console.log("[Audio] Audio startup complete");

        // RETRY MECHANISM: If music didn't start, retry periodically (silent retries)
        if (!this.spaceSound?.isPlaying && !this._retryInterval) {
            ...
            this._retryInterval = setInterval(() => {
                ...
                if (this.spaceSound && this.spaceSound.buffer && !this.spaceSound.isPlaying) {
                    ...
                    try {
                        this.spaceSound.play();
                        console.log('[Audio] Music started on retry');
                    } catch (e) {
                        // Silent retry
                    }
                }
