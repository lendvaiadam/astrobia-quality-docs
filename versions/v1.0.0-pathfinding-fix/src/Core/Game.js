...
export class Game {
    ...
    addWaypoint(point) {
        ...
        if (unit.waypointControlPoints.length === 0) {
            ...
            const startMarkerGeo = new THREE.SphereGeometry(0.8, 16, 16); // Larger for better hover detection
            const startMarkerMat = new THREE.MeshBasicMaterial({
                color: 0x00ff88,
                transparent: true,
                opacity: 0.7,
                depthTest: false, // Always visible - never hidden behind terrain
                depthWrite: false
            });
            ...
            startMarker.scale.setScalar(0.5); // Default: 50% size (large on hover)
        ...
        // Determine initial state: first target waypoint (index 1) should be 'approaching'
        const isFirstTarget = unit.waypoints.length === 1;
        
        unit.waypoints.push({ 
            id: newID, 
            position: newPoint,
            logicalState: isFirstTarget ? 'approaching' : 'neutral',
        ...
        const markerGeo = new THREE.SphereGeometry(0.8, 16, 16); // Larger for better hover detection
        ...
        marker.scale.setScalar(0.5); // Default: 50% size (large on hover)
    ...
    updateWaypointCurve() {
        ...
        const tangentScale = 0.25; // 25% of segment length (reduced from 40% to prevent loops)
    ...
    setupCommandQueueDragListeners() {
        ...
        deleteButtons.forEach((btn) => {
            btn.addEventListener('click', (e) => {
                ...
                console.log(`[UI] Delete waypoint at index ${index}`);
    ...
    deleteWaypointAtIndex(index) {
        const unit = this.getPanelUnit();
        if (!unit || !unit.waypointControlPoints) {
            console.log("[Game] deleteWaypointAtIndex: No unit");
            return;
        }

        // Bounds check
        if (index < 0 || index >= unit.waypointControlPoints.length) {
            console.warn(`[Game] Invalid waypoint index: ${index}`);
            return;
        }

        // Remove control point
        unit.waypointControlPoints.splice(index, 1);

        // Remove waypoint (ID data)
        if (unit.waypoints && unit.waypoints[index]) {
            unit.waypoints.splice(index, 1);
        }

        // Remove marker from scene and array
        if (unit.waypointMarkers && unit.waypointMarkers[index]) {
            const marker = unit.waypointMarkers[index];
            
            // Remove label sprite
            if (marker.userData.labelSprite) {
                this.scene.remove(marker.userData.labelSprite);
                if (marker.userData.labelSprite.material.map) {
                    marker.userData.labelSprite.material.map.dispose();
                }
                marker.userData.labelSprite.material.dispose();
            }
            ...
            unit.waypointMarkers.splice(index, 1);
        }

        // Update remaining marker indices and labels
        if (unit.waypointMarkers) {
            unit.waypointMarkers.forEach((marker, newIndex) => {
                marker.userData.waypointNumber = newIndex;
                marker.userData.controlPointIndex = newIndex;
                marker.userData.isStartMarker = (newIndex === 0);
        ...
        // If less than 2 points remain, stop path following
        if (unit.waypointControlPoints.length < 2) {
            unit.path = [];
            unit.isFollowingPath = false;
            ...
            console.log("[Game] Less than 2 waypoints remaining, path cleared");
        } else {
            // Regenerate path curve
            this.updateWaypointCurve();
        }
        ...
        console.log(`[Game] Deleted waypoint at index ${index}. Remaining: ${unit.waypointControlPoints.length}`);
    }
